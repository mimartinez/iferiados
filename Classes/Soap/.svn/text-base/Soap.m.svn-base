...
+(NSString*)serializeHeader:(NSDictionary*)headers forKey:(NSString*)key {
	id valueHeader = [headers objectForKey:key];
	
	// If its a literal, just output it
	if([valueHeader isKindOfClass:[SoapLiteral class]]) {
		return [valueHeader valueSoapLiteral];
	}
	
	// If it's a dictionary, then serialize and look for attributes
	if([valueHeader isKindOfClass:[NSDictionary class]]) {
		NSMutableString* attributes = [NSMutableString string];
		NSMutableString* elements = [NSMutableString string];
		for (id subkey in [valueHeader allKeys]) {
			if([subkey hasPrefix:@"@"]) {
				[attributes appendFormat:@" %@=\"%@\"", [subkey substringFromIndex:1], [valueHeader objectForKey:subkey]];
			} else {
				[elements appendFormat:@"<%@>%@</%@>", subkey, [Soap serialize: [valueHeader objectForKey:subkey]], subkey];
			}
		}
	...
	return [NSString stringWithFormat:@"<%@>%@</%@>", key, [Soap serialize: [valueHeader objectForKey: key]], key];
}
...
+ (id) deserialize: (CXMLNode*) element{
	...
	if(type == nil || type.length == 0) {
		...
		if(value == nil) {
			Class cls = NSClassFromString([NSString stringWithFormat:@"%@", type]);
			if(cls != nil ) {
